...
│   │   ├── document-store.ts    — TextChunk[] (чанкинг, keyword search, save/load)
...
- **Валидация ответа:** `generate()` проверяет `typeof data.response === "string"`; `chat()` проверяет `data.message?.content` — при невалидном ответе пробрасывается `data.error` или generic-сообщение
...
Настройка `indexingMode` в `NikelSettings` (dropdown в настройках, значения: `"vision" | "fast" | "direct"`):
...
- **`"direct"` (прямой):** извлечение текста (PDF→fast, TXT, DOCX) без EntityExtractor и KnowledgeGraph. Текст чанкуется и сохраняется в DocumentStore (`nikel/.nikel/document-store.json`). @nikel_s ищет по keyword-matching в чанках, LLM используется только для ответа на основе найденного контекста. Самая быстрая индексация (секунды), подходит когда не нужны сущности/связи и граф знаний.
...
### 9.10 DocumentStore (прямой режим)

**Файл:** `src/services/ingestion/document-store.ts`
**Хранилище:** `nikel/.nikel/document-store.json`

Класс для хранения и поиска текстовых блоков без использования LLM:

- **`addDocument(sourcePath, text, pageNum?)`** — добавляет текст, чанкуя его по 1000 символов с overlap 200.
- **`removeBySource(sourcePath)`** — удаляет все чанки из указанного файла.
- **`search(query, topK=5)`** — keyword search: токенизирует запрос в слова, для каждого чанка считает сколько слов запроса встречается, возвращает topK чанков с наибольшим совпадением.
- **`save()` / `load()`** — атомарная запись/чтение JSON (write→.tmp→rename).
- **`clear()`** — очищает все чанки.
- **`stats`** — `{ totalChunks, totalSources }`.

**Поток в `main.ts` при `indexingMode === "direct"`:**
1. `runIndexing()`: загружает DocumentStore → сканирует папки → для каждого файла извлекает текст → `addDocument()` → EntityExtractor и KnowledgeGraph пропускаются → `save()`.
2. `processNikelTask()`: `processWithDirectSearch()` → `documentStore.search(input)` → если чанки найдены, собирается контекст → `ollama.chat()` с system=контекст → ответ вставляется в редактор.

## 10. Важно помнить

- `165 тестов` (18 файлов, включая integration/pdf-renderer.test.ts, log-service.test.ts, text-extractor.test.ts, document-store.test.ts), build prod ~1.9MB (pdfjs-dist v4.10.38 + mammoth), tsc clean
