...
| `pdf-renderer (integration)` | `tests/integration/pdf-renderer.test.ts` | 3 — mocked pdfjs + canvas, тестирует Blob URL / Uint8Array / load→render→close |
...
- `PdfPageRenderer` — интерфейс, `DefaultPdfRenderer` — реализация через `pdfjs-dist v4.10.38`
...
1. `fileWatcher.scan(pdfFolder)` — рекурсивно ищет `.pdf` (только `.pdf`, `.docx` и другие игнорируются), сравнивает MD5 с `file-hashes.json`
2. Если изменений нет → `return`
3. `graph.load()` — читает `nikel/index.json`
4. Для каждого нового/изменённого файла:
   - `fs.readFile(filePath)` → `Uint8Array.from(raw)` (копия, не pool) → `pdfExtractor.extractPdf(data)` → Vision LLM → Markdown
...
- `127 тестов` (15 файлов, включая integration/pdf-renderer.test.ts), build prod ~1.9MB (pdfjs-dist v4.10.38), tsc clean
